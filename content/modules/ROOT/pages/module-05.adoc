...
* _get_last_result_for_cluster_: returns a dictionary with the cluster result.
* _get_upgrade_risk_prediction_for_cluster_: returns a dictionary with the upgrade risks prediction for the given cluster.
...
In order to deploy the MCP server, we will need to create several resources in our cluster. Some of them will be created via YAML in the OpenShift Dashboard, while others that require environment variables will be created from the Terminal.

Let's prepare the Web console to easily apply all the resources. In the upper-right corner menu, click on the *OpenShift command line*. This will open a new tab at the bottom of your page. There, select *Connect*. 
...
That's the last resource we will create using the Terminal, so you can close the Terminal window at the bottom.
...
In the Web Console interface, you will see a Pod being created that will reach the _Running_ status in a few seconds.
...
As we have seen in previous sections, the configuration in our OpenShift Lightspeed operator needs to be done in the OLSConfig resource. Navigate to *Ecosystem* and then *Installed Operators* in the left hand menu. Make sure you select *Project: All Projects* to list all the operators in the cluster. Click the *OpenShift Lightspeed Operator* and navigate to the *OLSConfig* tab. Then click on the `cluster` instance. Finally, select the *YAML* tab.
...
This new configuration will re-deploy the OpenShift Lightspeed service pod. Please wait a minute before starting to interact with OpenShift Lightspeed.
...
Ready to see how OpenShift Lightspeed handles virtualization workloads? Continue to the next module to explore OpenShift Lightspeed and Virtualization.